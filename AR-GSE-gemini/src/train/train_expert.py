...
CONFIG = {
    ...
    'experts_to_train': [
        {'name': 'ce', 'loss_type': 'ce'},
        {'name': 'logitadjust', 'loss_type': 'logitadjust'},
        {'name': 'balsoftmax', 'loss_type': 'balsoftmax'},
        # Thêm các expert khác nếu muốn, ví dụ: decoupled
    ],
    'train_params': {
        'epochs': 120, # Có thể giảm xuống để test nhanh
        'batch_size': 128,
        'lr': 0.1,
        'momentum': 0.9,
        'weight_decay': 5e-4,
...
def get_dataloaders(config):
    ...
    base_val_dataset = torchvision.datasets.CIFAR100(root=config['dataset']['data_root'], train=True, transform=transform_test)

    splits_dir = Path(config['dataset']['splits_dir'])
    with open(splits_dir / 'train_indices.json', 'r') as f:
        train_indices = json.load(f)
    with open(splits_dir / 'val_small_indices.json', 'r') as f:
        val_small_indices = json.load(f)

    train_dataset = Subset(base_train_dataset, train_indices)
    val_small_dataset = Subset(base_val_dataset, val_small_indices)

    train_loader = DataLoader(train_dataset, batch_size=config['train_params']['batch_size'], shuffle=True, num_workers=4)
    val_small_loader = DataLoader(val_small_dataset, batch_size=config['train_params']['batch_size'], shuffle=False, num_workers=4)
    
    return train_loader, val_small_loader

def get_loss_function(loss_type, train_loader):
    if loss_type == 'ce':
        return nn.CrossEntropyLoss()
    
    # For LT losses, we need class counts
...
def export_logits_for_all_splits(model, config, expert_name):
    ...
    # Use the simple test transform for all exports
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)),
    ])
    
    # We need to load indices for all splits
    splits_dir = Path(config['dataset']['splits_dir'])
    split_names = ['train', 'tuneV', 'val_small', 'calib']
    
    output_dir = Path(config['output']['logits_dir']) / config['dataset']['name'] / expert_name
    output_dir.mkdir(parents=True, exist_ok=True)

    # Process train-time splits
    ...
    for name in split_names:
        ...
        with torch.no_grad():
            for inputs, _ in tqdm(loader, desc=f"Exporting {name}"):
                ...
                logits = model.get_calibrated_logits(inputs) # Use calibrated logits
                all_logits.append(logits.cpu())
        
        all_logits = torch.cat(all_logits)
        torch.save(all_logits.to(torch.float16), output_dir / f"{name}_logits.pt")
        print(f"Saved {name} logits, shape: {all_logits.shape}")

    # Process test split (balanced)
    test_dataset = torchvision.datasets.CIFAR100(root=config['dataset']['data_root'], train=False, transform=transform)
    test_loader = DataLoader(test_dataset, batch_size=512, shuffle=False, num_workers=4)
    
    all_logits = []
    with torch.no_grad():
        for inputs, _ in tqdm(test_loader, desc="Exporting test"):
            inputs = inputs.to(DEVICE)
            logits = model.get_calibrated_logits(inputs)
            all_logits.append(logits.cpu())
            
    all_logits = torch.cat(all_logits)
    torch.save(all_logits.to(torch.float16), output_dir / "test_logits.pt")
    print(f"Saved test logits, shape: {all_logits.shape}")
...
def main():
    ...
    train_loader, val_small_loader = get_dataloaders(CONFIG)
    ...
    for expert_config in CONFIG['experts_to_train']:
        ...
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=CONFIG['train_params']['epochs'])
        ...
        for epoch in range(CONFIG['train_params']['epochs']):
            ...
            total = 0
            with torch.no_grad():
                for inputs, targets in val_small_loader:
                    inputs, targets = inputs.to(DEVICE), targets.to(DEVICE)
                    outputs = model(inputs)
        ...
        optimal_temp = scaler.fit(model, val_small_loader, DEVICE)
